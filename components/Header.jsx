      ...
      >
        ...
        <div className="lg:hidden bg-stone overflow-hidden" aria-hidden="true" style={{ maxHeight: mobileOpen ? '90vh' : '0', transition: 'max-height 0.4s ease', borderTop: mobileOpen ? '1px solid #e3e3d8' : 'none' }}>
          <div className="px-6 py-6 space-y-1 max-h-[80vh] overflow-y-auto">
            ...
            <button onClick={() => { setMobileOpen(false); window.dispatchEvent(new Event('open-quote-modal')); }} className="block border-2 border-gold bg-gold uppercase tracking-[0.16em] px-6 py-3 font-bold rounded-[20px] mt-4 text-center w-full cursor-pointer" style={{ fontSize: '0.75rem', transition: 'all 0.24s', color: '#fff' }}>
              Get Your Quote
            </button>
          </div>
        </div>
